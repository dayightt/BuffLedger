...
--   luajit tests/run.lua layout     run only files whose name contains "layout"
...
local CORE_FILES = {
    "Core.lua", "Categories.lua", "Data.lua", "Classify.lua", "Auras.lua", "Spells.lua", "Layout.lua",
    "Scan.lua", "Preview.lua",
}

local TEST_FILES = {
    "test_core", "test_categories", "test_data", "test_classify", "test_auras", "test_spells", "test_layout", "test_scan", "test_smoke",
}
...
function stub.LoadAddon(opts)
    ...
    stub.FireEvent("ADDON_LOADED", "BuffLedger")
    return BL
end
    ...
    if not filter or string.find(file, filter, 1, true) then
        ...
        if fh then
            ...
            if not chunk then
                ...
                io.write(string.format("%-16s %d tests\n", file, #tests))
            end
        end
    end
end
